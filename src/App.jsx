...
function App() {
  ...
  const handleTextChange=(e)=>{
    ...
    setNumberArray(Array.from({ length: lineCount }, (_, index) => index + 1))
  }
  return (
    <>
        ...
        <div style={{position:'fixed',width:'100%',height:'100%',display:'flex'}}>
            ...
            <div style={{width:'100%',height:'100vh',backgroundColor:'darkgrey',display:'flex',flexDirection:'column'}}>
                ...
                <div className="indexText" style={{cursor:'pointer',userSelect:'none',width:'2%',height:'100%',fontFamily:'sans-serif',resize:'none',border:'none',backgroundColor:'snow'}}>
                {numberArray.map((key,index)=>(<p key={index} style={{margin:'0rem',padding:'0rem',fontSize:'1rem'}}>{key}</p>))}
                </div>
                <textarea value={text} onChange={handleTextChange} style={{width:'98%',height:'100%',resize:'none',padding:'0rem',margin:'0rem',border:'none',overflowX:'scroll',whiteSpace: 'nowrap',fontFamily:'sans-serif',fontSize:'1rem'}}></textarea> 
                </div>
